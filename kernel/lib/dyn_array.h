...
#ifdef __GNUC__
#define DYNARR_UNUSED __attribute__((unused))
#else
#define DYNARR_UNUSED
#endif
...
// Special-case for wrapped static arrays
static inline size_t grow_x_0(size_t capacity, size_t elem_size) {
	(void)elem_size;
	return capacity;
}

#define dyn_foreach(var, arr) for (var = arr.items; var < (arr.items + arr.count); ++var)
...
#define dyn_array_def(T) \
	...
	static inline size_t DYNARR_UNUSED T##_arr_add(struct T##_arr *a, const T value) { \
		if (a->count >= a->capacity) { \
			size_t new_capacity = a->grow_fn ? a->grow_fn(a->capacity, sizeof(*a->items)) : grow_x_2(a->capacity, sizeof(*a->items)); \
			if (new_capacity == a->capacity) return a->count; \
	...
	static inline void DYNARR_UNUSED T##_arr_trim(struct T##_arr *a) { \
	...
	static inline struct T##_arr DYNARR_UNUSED T##_arr_copy(const struct T##_arr a) { \
	...
	static inline void DYNARR_UNUSED T##_arr_free(struct T##_arr *a) { \
		if (!a) return; \
		if (a->elem_free) { \
			for (size_t i = 0; i < a->count; ++i) \
				a->elem_free(&a->items[i]);\
		}\
		if (a->items && a->grow_fn != grow_x_0) free(a->items); \
		a->items = NULL; \
		a->capacity = 0; \
		a->count = 0; \
	} \
	static inline void DYNARR_UNUSED T##_arr_join(struct T##_arr *a, struct T##_arr *b) { \
		if (!a || !b || a->grow_fn == grow_x_0) return; \
